...
Sometimes, to keep our outputs more focused and easy to work with, we only keep the columns we care about.
...
Sometimes, we filter our data set to keep only the observations that meet certain conditions.
...
-   This works, but nested functions can be hard to read because you have to read the code inside-out. This is how I look trying to read code with deep nesting...
...
In data analysis, it's common to derive new variables from existing ones.
...
```{r}
penguins |> 
  mutate(flipper_size = if_else(flipper_length_mm >= 200, "long", "short"))
```
...
-   Use `if_else()` when you only have two options. Use `case_when()` when you need to handle multiple conditions and return different values for each.
...
We often need to summarize our data to understand its key characteristics (e.g., measures of central tendency and variation).
...
A common task in data analysis is to split the data into groups, apply a function to each group, and then combine the results. This is known as the split-apply-combine paradigm.
...
#### *Extract the largest body mass*

1.  Use `arrange()` to sort the data by body mass in descending order.
2.  Use `slice()` to select the row with the largest body mass.
3.  Use `pull()` to extract the value as a numeric vector.
...
-   Tip: `dplyr` also provides helper functions such as [`slice_min()`](https://dplyr.tidyverse.org/reference/slice.html) to simply row selection for common use cases.
...
By default, the `penguins` data set is arranged so that each row represents a penguin. But what if we wanted our observation of interest to be the measurement itself, rather than the penguin? That's to say, we want to restructure the data to have the columns `id` (unique identifier of penguin), `measurement` and `value`.
